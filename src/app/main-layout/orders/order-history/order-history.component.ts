...
export class OrderHistoryComponent implements OnInit {
  ...
  constructor(
    ...
    this.service.post('CheckOrderStatus', body).subscribe(res => {
      ...
      if (data.CheckOrderStatusResponse && data.CheckOrderStatusResponse.Found._text && data.CheckOrderStatusResponse.Orders.Order.length) {
    ...
    this.service.post('CheckOrderStatus', body1).subscribe(res => {
      ...
      if (data.CheckOrderStatusResponse && data.CheckOrderStatusResponse.Found._text && data.CheckOrderStatusResponse.Orders && data.CheckOrderStatusResponse.Orders.order && data.CheckOrderStatusResponse.Orders.Order.length) {
        ...
        data.CheckOrderStatusResponse.Orders.Order.OrderDueDate._text = moment(data.CheckOrderStatusResponse.Orders.Order.OrderDueDate._text, 'YYYY-MM-DD').format('DD MMM YYYY');
        this.completeOrder.push(data.CheckOrderStatusResponse.Orders.Order);

      } else {
        this.isnodata = true;
        this.message = 'There is no Completed Orderes.';
      }
